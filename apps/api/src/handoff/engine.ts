...
export async function requestHandoffIfNeeded(conversationId: string) {
  ...
  if (!withinHours) {
    await prisma.conversation.update({ where: { id: conversationId }, data: { leadStatus: 'HOT_WAITING' as LeadStatus } });
    await prisma.conversationEvent.create({ data: { conversationId, kind:'HANDOFF_WAITING_AFTER_HOURS', payload: {} } });
    return;
  }
